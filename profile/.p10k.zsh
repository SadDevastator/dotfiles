# Generated by Powerlevel10k configuration wizard on 2026-08-25 at 02:52 EEST.
...
() {
  ...
  # ---------------------------------------------------------------------------
  # Horizontal-resize fix.
  #
  # When the terminal is narrowed (e.g. Hyprland re-tiling a second window in
  # beside this one), WezTerm rewraps the lines already on screen. ZLE then
  # repaints the prompt using its own row accounting, which knows nothing about
  # that rewrap, so the extra row it created is left behind as a stale copy of
  # the prompt header.
  #
  # A right-aligned first line is always padded out to the full terminal width,
  # so it always exceeds the new width and always rewraps. Rendering the same
  # segments inline on the left instead keeps the line's length tied to its
  # content, which stays well short of the width and so never rewraps.
  #
  # Verified: with the right-aligned layout a 91->45 column shrink leaves two
  # prompt headers on screen; with this layout it leaves one.
  # ---------------------------------------------------------------------------
  typeset -g POWERLEVEL9K_LEFT_PROMPT_ELEMENTS=(
    os_icon
    dir
